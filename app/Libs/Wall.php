...
//            $textLength = count($expBR);
			...
			if ($row['attach']) {
				...
				if ($resLinkTitle and $row['text'] == $resLinkUrl or !$row['text']) $row['text'] = $resLinkTitle . $attach_result; elseif ($attach_result) $row['text'] = preg_replace('`(http(?:s)?://\w+[^\s\[\]\<]+)`i', '<a href="/away.php?url=$1" target="_blank">$1</a>', $row['text']) . $attach_result; else
...
//                    $dateTell = 'N/A';
			...
			if ($action_type == 1) {
				...
				$query[$key]['online'] = Tools::Online($row['user_last_visit']);
