...
function audioAddedLoadAjax(){
	...
	$.post('/public_audio/search/', {page: page_cnt, query: $('#query_audio').val(), pid: '{pid}'}, function(d){
...
function ListAudioAddedLoadAjax(){
	...
	$.post('/public_audio/{pid}/', {page: xpage_cnt}, function(d){
...
function PublicAudioSearch(){
	if($('#query_audio').val() != 'Поиск по композициям и исполнителям' && $('#query_audio').val() != 0){
		...
		$.post('/public_audio/search/', {query: $('#query_audio').val(), adres: '{adres}', pid: '{pid}'}, function(d){
			$('#allGrAudis').hide();
...
function GroupAudioAddMyList(aid){
		...
		$.post('/public_audio/addlistgroup/', {aid: aid, pid: '{pid}'});
}
function PublicAudioEditsave(aid, pid){
	...
	$.post('/public_audio/editsave/', {aid: aid, artist: $('#valartis'+aid).val(), name: $('#vaname'+aid).val(), pid: pid});
	Box.Close();
}
function PublicAudioDelete(aid){
	Page.Loading('start');
	$('.js_titleRemove').hide();
	$.post('/public_audio/del/', {aid: aid, pid: '{pid}'}, function(){
