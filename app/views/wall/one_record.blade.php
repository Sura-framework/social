@foreach($wall_records as $row)
    <div class="card mt-3 wallrecord" id="wall_record_{{ $row['id'] }}">
        ...
        <div class="card-body">
            <div class="mb-3">
                <div class="row align-items-center">
                    <div class="col-auto @if($row['privacy_comment'] AND $row['if_comments']) wall_ava_mini @endif " id="ava_rec_{{ $row['id'] }}">
                        <a href="/u{{ $row['user_id'] }}" class="avatar"  onmouseover="wall.showTag({{ $row['user_id'] }}, {{ $row['id'] }}, {{ $row['action_type'] }})" onmouseout="wall.hideTag({{ $row['user_id'] }}, {{ $row['id'] }}, {{ $row['action_type'] }})">
                            {{-- <img src="{{ $row['ava'] }}" alt="..." class="avatar-img rounded-circle"> --}}

                            <div class="wall-avatar wall-avatar-online mr-5">
                                <img src="{{ $row['ava'] }}" alt="{{ $row['name'] }}" class="wall-avatar-img rounded-circle" style="width: 34px;height: 34px;" onmouseover="wall.showTag({{ $row['user_id'] }}, {{ $row['id'] }}, {{ $row['action_type'] }})" onmouseout="wall.hideTag({{ $row['user_id'] }}, {{ $row['id'] }}, {{ $row['action_type'] }})">
                            </div>
                        </a>
                    </div>
                    <div class="col ml-n2">
                        <h4 class="mb-1" onmouseover="wall.showTag({{ $row['user_id'] }}, {{ $row['id'] }}, {{ $row['action_type'] }})" onmouseout="wall.hideTag({{ $row['user_id'] }}, {{ $row['id'] }}, {{ $row['action_type'] }})">{{ $row['name'] }}</h4>
                        <p class="card-text small text-muted">
                            {{ $row['action_type'] }}|{{ $row['type'] }} <time datetime="2018-05-24"> {{ $row['date'] }}</time>
                        </p>
                    </div>
                    <div class="col-auto">
                        @if($row['action_type'] == 1)
                            ...
                            @endif
                            <div class="wall_tell_all cursor_pointer" onMouseOver="myhtml.title('{{ $row['id'] }}', 'Отправить в сообщество или другу', 'wall_tell_all_')" onClick="Repost.Box('{{ $row['id'] }}'); return false "id="wall_tell_all_{{ $row['id'] }}"></div>
                        ...
                        @else
                            @if($row['owner'])
                                <div class="wall_delete" onMouseOver="myhtml.title('{{ $row['id'] }}', 'Удалить запись', 'wall_del_')" onClick="groups.delet('{{ $row['id'] }}'); return false" id="wall_del_{{ $row['id'] }}"></div>
                            @endif
                            <div class="wall_tell_all cursor_pointer" onMouseOver="myhtml.title('{{ $row['id'] }}', 'Отправить в сообщество или другу', 'wall_tell_all_')" onClick="Repost.Box('{{ $row['id'] }}'); return false "id="wall_tell_all_{{ $row['id'] }}"></div>
                            @if($row['author_user_id'])
                ...
                <div class="public_likes_user_block no_display"
                    ...
                    <div class="public_wall_likes_hidden">
                        <div class="public_wall_likes_hidden2">
                            <a href="/u{{ $row['viewer_id'] }}" id="like_user{viewer_id}_{{ $row['id'] }}" class="no_display" onClick="Page.Go(this.href); return false">
        ...
        <div class="wall_fast_opened_form" id="fast_form" style="margin: 0;padding: 20px 10px;">
            ...
            onKeyPress="if(event.keyCode == 10 || (event.ctrlKey && event.keyCode == 13))wall.fast_send('{{ $row['id'] }}', '{author-id}', 2)"></textarea>
                <div class="button_div fl_l margin_top_5"><button onClick="wall.fast_send('{{ $row['id'] }}', '{{ $row['user_id'] }}', 2); return false" id="fast_buts_{{ $row['id'] }}">Отправить</button></div>
