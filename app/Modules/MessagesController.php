            ...
            if(isset($msg) AND !empty($msg) OR isset($attach_files) OR !empty($attach_files)){
                ...
                if($row){
                    ...
                    if($xPrivasy AND $user_id != $for_user_id){
                        ...
                        $db->query("INSERT INTO `messages` SET theme = '{$theme}', text = '{$msg}', for_user_id = '{$user_id}', from_user_id = '{$for_user_id}', date = '{$server_time}', pm_read = 'no', folder = 'outbox', history_user_id = '{$user_id}', attach = '".$attach_files."'");
...
//            Tools::NoAjaxQuery();
            ...
            $row = $db->super_query("SELECT pm_read, from_user_id FROM `messages` WHERE id = '{$mid}' AND for_user_id = '{$user_id}' AND folder = '{$folder}'");
            if($row){
                //Удаляе само сообщение
                $db->query("DELETE FROM `messages` WHERE id = '{$mid}' AND folder = '{$folder}' AND for_user_id = '{$user_id}'");
                ...
                if($row['pm_read'] == 'no' AND $folder == 'inbox'){
                    ...
                    $db->query("UPDATE `im` SET msg_num = msg_num-1, all_msg_num = all_msg_num-1 WHERE iuser_id = '".$user_id."' AND im_user_id = '".$row['from_user_id']."'");
                } else
                    $db->query("UPDATE `im` SET all_msg_num = all_msg_num-1 WHERE iuser_id = '".$user_id."' AND im_user_id = '".$row['from_user_id']."'");
            }
        ...
        if($logged){
            ...
            if($mid){
                ...
                if($row){
                    ...
                    if($row['pm_read'] == 'no' AND $folder == 'inbox'){
                        $db->query("UPDATE `messages` SET pm_read = 'yes' WHERE id = '{$mid}'");
                        $db->query("UPDATE `messages` SET pm_read = 'yes' WHERE id = '".($mid+1)."'");
                        $db->query("UPDATE `users` SET user_pm_num = user_pm_num-1 WHERE user_id = '{$user_id}'");
                        $db->query("UPDATE `im` SET msg_num = msg_num-1 WHERE iuser_id = '".$user_id."' AND im_user_id = '".$row['from_user_id']."'");
...
//            Tools::NoAjaxQuery();
    ...
    public function outbox($params){
        ...
        if($logged){
            ...
            $query = "SELECT tb1.id, theme, text, from_user_id, date, pm_read, attach, tb2.user_search_pref, user_photo, user_last_visit, user_logged_mobile FROM `messages` tb1, `users` tb2 WHERE tb1.for_user_id = '{$user_id}' AND tb1.from_user_id = tb2.user_id {$search_sql} AND  tb1.folder = 'outbox' ORDER by `date` DESC LIMIT {$limit_page}, {$gcount}";
            $sql_ = $db->super_query($query, 1);

            if($sql_)
                $msg_count = $db->super_query("SELECT COUNT(*) AS cnt FROM `messages` tb1, `users` tb2 WHERE tb1.for_user_id = '{$user_id}' AND tb1.from_user_id = tb2.user_id {$search_sql} AND tb1.folder = 'outbox'");
...
//                    msg_box('', '<script type="text/javascript">setTimeout(\'$(".err_yellow").fadeOut()\', 1500);</script>Ваше сообщение успешно отправлено.', 'info');
                ...
                $query = "SELECT tb1.id, theme, text, for_user_id, from_user_id, date, pm_read, attach, tb2.user_search_pref, user_photo, user_last_visit, user_logged_mobile FROM `messages` tb1, `users` tb2 WHERE tb1.for_user_id = '{$user_id}' AND tb1.folder = 'inbox' AND tb1.from_user_id = tb2.user_id {$search_sql} ORDER by `date` DESC LIMIT {$limit_page}, {$gcount}";
                $sql_ = $db->super_query($query, 1);
