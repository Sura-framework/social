						...
						foreach($sql_ as $row){
							...
							//Если страница ответов "стена"
								...
								if($row_info OR $row['answer_text']){
									...
									if($row_info['attach']){
										...
										foreach($attach_arr as $attach_file){
											...
											//Музыка
												...
												$audioInfo = $db->super_query("SELECT artist, title, url FROM `audio` WHERE oid = '".$audioId."'");
												if($audioInfo){
									...
									if($row_info['tell_uid']){
										if($row_info['public'])
											$rowUserTell = $db->super_query("SELECT title, photo FROM `communities` WHERE id = '{$row_info['tell_uid']}'", false, "wall/group{$row_info['tell_uid']}");
										else
								...
								if($row_info_likes){
									...
									if($row_info_likes['tell_uid']){
										if($row_info_likes['public'])
											$rowUserTell = $db->super_query("SELECT title, photo FROM `communities` WHERE id = '{$row_info_likes['tell_uid']}'", false, "wall/group{$row_info_likes['tell_uid']}");
										else
							...
							if($row['action_type'] == 1){
								...
								if($rec_info['tell_uid']){
									if($rec_info['public'])
										$rowUserTell = $db->super_query("SELECT title, photo FROM `communities` WHERE id = '{$rec_info['tell_uid']}'", false, "wall/group{$rec_info['tell_uid']}");
									else
								...
								if($user_privacy['val_wall3'] == 1 OR $user_privacy['val_wall3'] == 2 AND $check_friend OR $user_id == $row['ac_user_id']){
									...
									if($rec_info['fasts_num']){
										...
										$sql_comments = $db->super_query("SELECT tb1.id, author_user_id, text, add_date, tb2.user_photo, user_search_pref FROM `wall` tb1, `users` tb2 WHERE tb1.author_user_id = tb2.user_id AND tb1.fast_comm_id = '{$row['obj_id']}' ORDER by `add_date` ASC LIMIT {$comments_limit}, 3", 1);
								...
								if($rec_info_groups['tell_uid']){
									if($rec_info_groups['public'])
										$rowUserTell = $db->super_query("SELECT title, photo FROM `communities` WHERE id = '{$rec_info_groups['tell_uid']}'", false, "wall/group{$rec_info_groups['tell_uid']}");
									else
								...
								if($rowInfoUser['comments']){
									...
									if($rec_info_groups['fasts_num']){
										...
										$sql_comments = $db->super_query("SELECT tb1.id, public_id, text, add_date, tb2.user_photo, user_search_pref FROM `communities_wall` tb1, `users` tb2 WHERE tb1.public_id = tb2.user_id AND tb1.fast_comm_id = '{$row['obj_id']}' ORDER by `add_date` ASC LIMIT {$comments_limit}, 3", 1);
