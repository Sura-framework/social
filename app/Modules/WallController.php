...
//                            $wall = new \wall();
                            ...
                            if(!$fast_comm_id){
                                ...
                                if($xPrivasyX){
                                    ...
                                    $query = $db->super_query("SELECT tb1.id, author_user_id, add_date, fasts_num, likes_num, likes_users, type, tell_uid, tell_date, public, attach, tell_comm, tb2.user_photo, user_search_pref, user_last_visit, user_logged_mobile FROM `wall` tb1, `users` tb2 WHERE for_user_id = '{$for_user_id}' AND tb1.author_user_id = tb2.user_id AND tb1.fast_comm_id = '0' ORDER by `add_date` DESC LIMIT 0, {$limit_select}", 1);
    ...
    public function liked_users($params){
        ...
        if($logged){
            ...
            Tools::NoAjaxQuery();
            $rid = intval($_POST['rid']);
            $sql_ = $db->super_query("SELECT tb1.user_id, tb2.user_photo FROM `wall_like` tb1, `users` tb2 WHERE tb1.user_id = tb2.user_id AND tb1.rec_id = '{$rid}' ORDER by `date` DESC LIMIT 0, 7", 1);
            if($sql_){
                foreach($sql_ as $row){
    ...
    public function all_liked_users($params){
        ...
        if($logged){
            ...
            if($rid AND $liked_num){
                $sql_ = $db->super_query("SELECT tb1.user_id, tb2.user_photo, user_search_pref FROM `wall_like` tb1, `users` tb2 WHERE tb1.user_id = tb2.user_id AND tb1.rec_id = '{$rid}' ORDER by `date` DESC LIMIT {$limit_page}, {$gcount}", 1);
