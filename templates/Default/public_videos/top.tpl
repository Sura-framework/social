...
function videoAddedLoadAjax(){
  ...
  $.post('/public_videos/search/', {page: page_cnt, query: $('#query_audio').val(), pid: '9'}, function(d){
...
function PublicVideoSearch(){
  if($('#query_video').val() != 'Поиск видеозаписей по названию' && $('#query_video').val() != 0){
    ...
    $.post('/public_videos/search/', {query: $('#query_video').val(), adres: '{adres}', pid: '{pid}'}, function(d){
	  page_cnt = 1;
      $('#allGrAudis').hide();
...
function delVideoOutPublic(i, p){
  ...
  $.post('/public_videos/del/', {id: i, pid: p});
}
function videoPubEditBox(i, p){
  Box.Page('/public_videos/edit/', 'id='+i+'&pid='+p, 'edit_video', 510, lang_video_edit, lang_box_canсel, lang_box_save, 'videoPubEditSave('+i+', '+p+'); return false', 255, 0, 1, 1, 0);
}
function videoPubEditSave(i, p){
  ...
  $('#box_but').hide();
  $('#box_loading').fadeIn();
  $.post('/public_videos/edit_save/', {id: i, pid: p, title: title, descr: descr}, function(d){
